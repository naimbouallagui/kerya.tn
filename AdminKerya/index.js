...
<thead>
  ...
  for (let i = 0; i < clients.length; i++) {
    j+=1;
          html += `  <tr>
      <td style="width: 1%">${j}</td>
      <td style="width: 20%">${clients[i].username}</td>
      <td style="width: 30%">${clients[i].adress}</td>
      <td class="project-actions" style="width: 20%">
      ${clients[i].email}
        </td>
      <td class="project-actions" style="width: 20%">
  ...
  html += ` </table>`;
  // inject html
  document.getElementById("clients_table").innerHTML = html;
}
